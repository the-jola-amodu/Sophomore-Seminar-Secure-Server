...
def home():
    ...
    if request.method == "POST":
        user_name = str(escape(request.form.get("name")))
...
def message(data):
    ...
    if name not in last_message_time:
        last_message_time[name] = []
    
    current_time = time()
    last_message_time[name] = [t for t in last_message_time[name] if current_time - t < LIMIT_INTERVAL]
    if len(last_message_time[name]) >= LIMIT_COUNT:
        print(f"RATE LIMIT EXCEEDED: {name}")
        return
    last_message_time[name].append(current_time)

    user_message = str(escape(data["data"]))
